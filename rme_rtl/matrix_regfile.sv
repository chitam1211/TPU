module matrix_regfile #(
    ...
    //
    // Rows are stored as packed vectors instead of a third unpacked beat dimension.
    // This avoids simulator X issues on reset/read with multidimensional unpacked
    // arrays while preserving the logical access pattern: register -> row -> 32-bit beat.
    localparam int TR_ROW_BITS  = BEATS_PER_ROW * 32;
    localparam int ACC_ROW_BITS = ACC_BEATS_PER_ROW * 32;

    logic [TR_ROW_BITS-1:0]  tr_regs  [0:3][0:MATRIX_DIM-1];
    logic [ACC_ROW_BITS-1:0] acc_regs [0:3][0:MATRIX_DIM-1];
        ...
        if (!resetn) begin
            ...
            for (int i = 0; i < 4; i++) begin
                if (tr_wen_oh[i]) begin
                    tr_regs[i][reg_row_idx][reg_beat_idx * 32 +: 32] <= reg_wdata;
                end

                if (acc_wen_oh[i]) begin
                    acc_regs[i][reg_row_idx][reg_beat_idx * 32 +: 32] <= reg_wdata;
                end
            end
        end
    end
        ...
        begin
            ...
            if (!id[2]) begin
                if ((row < MATRIX_DIM) && (beat < BEATS_PER_ROW)) begin
                    read_word = tr_regs[id[1:0]][row][beat * 32 +: 32];
                end
            end else begin
                if ((row < MATRIX_DIM) && (beat < ACC_BEATS_PER_ROW)) begin
                    read_word = acc_regs[id[1:0]][row][beat * 32 +: 32];
                end
            end
        end
    endfunction
    ...
    assign reg_rdata        = read_word(dma_read_id, dma_read_row, dma_read_beat);
    assign reg_mac_rdata_A  = read_word(mac_read_id_A, mac_read_row_A, mac_read_beat_A);
    assign reg_mac_rdata_B  = read_word(mac_read_id_B, mac_read_row_B, mac_read_beat_B);
    assign reg_mac_rdata_C  = read_dword(mac_read_id_C, mac_read_row_C, mac_read_beat_C);
