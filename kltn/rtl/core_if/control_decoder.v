module control_decoder (
    ...
    input wire auipc,
    input wire matrix,
    input wire fp_load_type,
    input wire fp_store_type,
...
always @(*) begin
    // Defined inactive controls: unsupported encodings must not infer latches.
    imm_sel = 3'b000;
    mem_to_reg = 2'b00;
    alu_control = 4'b0000;
    // RV32F memory operations use word-sized operands only.
    fp_load = fp_load_type & (fun3 == 3'b010);
    fp_store = fp_store_type & (fun3 == 3'b010);
    fp_reg_write = fp_load;
    ...
    operand_b = i_type | load | store | fp_load | fp_store | branch | jal | jalr | lui | auipc;
    //load
    Load = load | fp_load;
    //store
    Store = store | fp_store;
    ...
    else if (store | fp_store) begin //store / FSW
    ...
    else if (load | fp_load) begin //load / FLW
        imm_sel = 3'b000; //i_type selection
        mem_to_reg = 2'b01;
    ...
    if(jalr)begin
        mem_to_reg = 2'b10; // JALR writes PC+4, independently of its jump target.
        alu_control = 4'b0000;
        imm_sel = 3'b000;//i_type selection
    end
    else if(lui)begin
