...
class LoadStoreLogic:
    ...
    def execute_load_store(self, instruction):
        ...
        # The current RTL exposes four 32-bit beats per physical register row.
        # INT8 A/B transfers therefore pack up to four elements in each beat,
        # while C transfers one INT32 element per beat. Keep this path aligned
        # with matrix_tile_ls.sv for the six instructions implemented in RTL.
        if func4 in ("0000", "0001") and d_size_str == "00":
            rows = M if func4 == "0000" else N
            row_bytes = K
            beats_per_row = min(4, (row_bytes + 3) // 4)
            target_reg = self.get_matrix_reg_int(reg_idx)

            for row in range(min(rows, 4)):
                for beat in range(beats_per_row):
                    mem_addr = base_addr + row * row_stride + beat * 4
                    remaining = max(0, row_bytes - beat * 4)
                    valid_bytes = min(4, remaining)
                    if is_load:
                        raw = self.memory.read(mem_addr, 4)
                        target_reg[row][beat] = int.from_bytes(raw, "little")
                    else:
                        word = int(target_reg[row][beat]) & 0xFFFFFFFF
                        raw = word.to_bytes(4, "little")
                        self.memory.write(mem_addr, raw[:valid_bytes])
            return

        if func4 == "0010" and d_size_str == "10":
            target_reg = self.get_matrix_reg_int(reg_idx)
            for row in range(min(M, 4)):
                for col in range(min(N, 4)):
                    mem_addr = base_addr + row * row_stride + col * 4
                    if is_load:
                        raw = self.memory.read(mem_addr, 4)
                        target_reg[row][col] = int.from_bytes(raw, "little")
                    else:
                        word = int(target_reg[row][col]) & 0xFFFFFFFF
                        self.memory.write(mem_addr, word.to_bytes(4, "little"))
            return
        ...
        # mlce8/16/32 / msce8/16/32 (Matrix C, non-transposed)
            ...
            # Matrix C is row-major in memory: mem_addr = base + row*stride + col*element_size
