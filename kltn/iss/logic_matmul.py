...
class MatmulLogic:
    ...
    def execute_matmul(self, instruction):
        ...
        if is_float_op:
            self.acc_dest_bits_float[acc_idx] = dest_bits
        else:
            self.acc_dest_bits_int[acc_idx] = dest_bits
        ...
        for m in range(M):
            for n in range(N):
                if is_float_op:
                    ...
                    c_old_quantized = int(mat_C_old[m][n])

                dot_product = 0.0 if is_float_op else 0
                for k in range(K):
