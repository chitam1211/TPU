...
class MiscLogic:
    ...
    def _exec_mmov_x_m(self, rd_idx, ms2_idx, rs1_val, ctrl_size):
        ...
        src_array = self._get_reg_array_by_idx(ms2_idx, is_float=False)
        ...
        self.gpr_ref.write(rd_idx, src_array[row_idx][col_idx])
    ...
    def _exec_mmov_m_x_or_mdup(self, md_idx, rs2_val, rs1_val, ctrl_bit_25, d_size_str):
        ...
        eew = 32
        dest_array = self._get_reg_array_by_idx(md_idx, is_float=False)
        ...
        if ctrl_bit_25 == '1': # mmovw.m.x: Ghi 1 phần tử 
            ...
            dest_array[row_idx][col_idx] = rs2_val & 0xFFFFFFFF

        else: # mdupw.m.x: Sao chép rs2 ra toàn bộ thanh ghi
            print(f"    - Executing mdupw.m.x (md={md_idx})")
            val = rs2_val & 0xFFFFFFFF
            for i in range(rows):
                for j in range(cols_phys):
                    dest_array[i][j] = val

    def _exec_slide(self, md_idx, ms1_idx, imm3, slide_type):
        """Execute a zero-filling row or column slide."""
        print(f"    - Executing {slide_type} (md={md_idx}, ms1={ms1_idx}, imm3={imm3})")
        
        src_array = self._get_reg_array_by_idx(ms1_idx, is_float=False)
        dest_array = self._get_reg_array_by_idx(md_idx, is_float=False)
        rows, cols = self._get_reg_dims_by_idx(md_idx)
        snapshot = [row[:] for row in src_array]
        
        if slide_type == 'row_down':
            for i in range(rows):
                for j in range(cols):
                    src_row = i + imm3
                    dest_array[i][j] = snapshot[src_row][j] if src_row < rows else 0
        
        elif slide_type == 'col_down':
            for i in range(rows):
                for j in range(cols):
                    src_col = j + imm3
                    dest_array[i][j] = snapshot[i][src_col] if src_col < cols else 0

        elif slide_type == 'row_up':
            for i in range(rows):
                for j in range(cols):
                    src_row = i - imm3
                    dest_array[i][j] = snapshot[src_row][j] if src_row >= 0 else 0

        elif slide_type == 'col_up':
            for i in range(rows):
                for j in range(cols):
                    src_col = j - imm3
                    dest_array[i][j] = snapshot[i][src_col] if src_col >= 0 else 0
    ...
    def execute_misc(self, instruction):
        ...
        elif func4 == "0110" and uop == "11":
            if s_size_str == "00" and d_size_str == "00":
                self._exec_slide(md_idx, ms1_idx, ctrl_imm3, 'row_up')
            else:
                print(f"  -> ERROR: Only mrslideup (s_size=00, d_size=00) is supported")
                return
        ...
        # Lệnh 7: mcslidedown.w
            ...
            if s_size_str == "10" and d_size_str == "10":
                self._exec_slide(md_idx, ms1_idx, ctrl_imm3, 'col_up')
            else:
                print(f"  -> ERROR: Only mcslideup.w (s_size=10, d_size=10) is supported")
                return
